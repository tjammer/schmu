Simplest module with 1 type and 1 nonpolymorphic function
  ...
  %std.option_float = type { i32, double }
  %std.option_int = type { i32, i64 }
  
  @schmu_none = global %std.option_float zeroinitializer, align 16
  @0 = private unnamed_addr constant { i64, i64, [4 x i8] } { i64 3, i64 3, [4 x i8] c"%i\0A\00" }
  
  declare void @printf(i8* %0, i64 %1)
  
  define linkonce_odr i64 @__std.optiong.i_poly_func_classify_std.optionf.i(%std.option_float* %thing) {
  entry:
    %tag1 = bitcast %std.option_float* %thing to i32*
    %index = load i32, i32* %tag1, align 4
    %eq = icmp eq i32 %index, 0
    br i1 %eq, label %then, label %ifcont
  
  then:                                             ; preds = %entry
    %data = getelementptr inbounds %std.option_float, %std.option_float* %thing, i32 0, i32 1
    br label %ifcont
  ...
  define linkonce_odr i64 @__std.optiong.i_poly_func_classify_std.optioni.i(%std.option_int* %thing) {
  entry:
    %tag1 = bitcast %std.option_int* %thing to i32*
    %index = load i32, i32* %tag1, align 4
    %eq = icmp eq i32 %index, 0
    br i1 %eq, label %then, label %ifcont
  
  then:                                             ; preds = %entry
    %data = getelementptr inbounds %std.option_int, %std.option_int* %thing, i32 0, i32 1
    br label %ifcont
  ...
  entry:
    ...
    %tag4 = bitcast %std.option_int* %option to i32*
    ...
    %0 = call i64 @__std.optiong.i_poly_func_classify_std.optioni.i(%std.option_int* %option)
    call void @printf(i8* getelementptr (i8, i8* bitcast ({ i64, i64, [4 x i8] }* @0 to i8*), i64 16), i64 %0)
    %option1 = alloca %std.option_float, align 8
    %tag25 = bitcast %std.option_float* %option1 to i32*
    store i32 0, i32* %tag25, align 4
    %data3 = getelementptr inbounds %std.option_float, %std.option_float* %option1, i32 0, i32 1
    store double 3.000000e+00, double* %data3, align 8
    %1 = call i64 @__std.optiong.i_poly_func_classify_std.optionf.i(%std.option_float* %option1)
    call void @printf(i8* getelementptr (i8, i8* bitcast ({ i64, i64, [4 x i8] }* @0 to i8*), i64 16), i64 %1)
    store i32 1, i32* getelementptr inbounds (%std.option_float, %std.option_float* @schmu_none, i32 0, i32 0), align 4
    %2 = call i64 @__std.optiong.i_poly_func_classify_std.optionf.i(%std.option_float* @schmu_none)
