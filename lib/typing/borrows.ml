...
let rec check_expr st ac part ctx tyex =
  ...
  | App { callee; args } ->
      ...
      (match ctx with
      | Clet when Subscript.is_borrow_call_typed callee.expr ->
          raise
            (Error
               ( tyex.loc,
      ...
      let (_, tmpstate, trees), nargs =
        List.fold_left_map
          (fun (i, tmpstate, trees) (arg, attr, mode) ->
            ...
            in
            let _, _, tmpstate =
              check_let tmpstate ~attr:`Expr id arg.loc attr lmut `Arg arg
            in
  ...
  | Let ({ id; lmut; pass; rhs; cont; id_loc; mode; borrow_app; _ } as lt) ->
      let attr = if borrow_app then `Expr else `Let in
      let rhs, pass, st =
        check_let st ~attr id id_loc pass lmut (let_mode mode) rhs
      in
...
and check_let st ~attr str loc pass lmut mode rhs =
  let tl = attr = `Toplevel in
  (match pass with
  | Dmut when tl -> raise (Error (rhs.loc, "Cannot project at top level"))
  ...
  let ctx = match attr with `Toplevel | `Let -> Clet | `Expr -> Cexpr in
  let rhs, pass, trees =
...
let check_item st = function
  | Tl_let ({ id; pass; rhs; lmut; loc; _ } as tl) ->
      let rhs, pass, st =
        check_let ~attr:`Toplevel st id loc pass lmut `Many rhs
      in
